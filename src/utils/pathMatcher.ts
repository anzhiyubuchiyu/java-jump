...
export interface PathMatchRank {
  namespaceExact: boolean;
  sameModule: boolean;
  sourceSetMatch: boolean;
  commonPathDepth: number;
  fileNameExact: boolean;
  pathSimilarity: number;
}

export class PathMatcher {
  static createMatchRank(
    referencePath: string,
    candidatePath: string,
    namespaceExact = true
  ): PathMatchRank {
    const referenceParts = this.normalizeSeparators(referencePath).toLowerCase().split('/');
    const candidateParts = this.normalizeSeparators(candidatePath).toLowerCase().split('/');
    const referenceName = referenceParts[referenceParts.length - 1].replace(/\.\w+$/, '');
    const candidateName = candidateParts[candidateParts.length - 1].replace(/\.\w+$/, '');
    const referenceLogicalParts = this.extractLogicalDirectoryParts(referencePath);
    const candidateLogicalParts = this.extractLogicalDirectoryParts(candidatePath);
    const maxDepth = Math.min(referenceLogicalParts.length, candidateLogicalParts.length);
    let commonPathDepth = 0;
    for (let index = 0; index < maxDepth; index++) {
      if (referenceLogicalParts[index] !== candidateLogicalParts[index]) break;
      commonPathDepth++;
    }

    return {
      namespaceExact,
      sameModule: this.isSameModule(referencePath, candidatePath),
      sourceSetMatch: this.isSameSourceSet(referencePath, candidatePath),
      commonPathDepth,
      fileNameExact: referenceName === candidateName,
      pathSimilarity: this.calculateSimilarity(referencePath, candidatePath)
    };
  }

  static compareMatchRanks(left: PathMatchRank, right: PathMatchRank): number {
    const compareBoolean = (a: boolean, b: boolean) => Number(b) - Number(a);
    return compareBoolean(left.namespaceExact, right.namespaceExact) ||
      compareBoolean(left.sameModule, right.sameModule) ||
      compareBoolean(left.sourceSetMatch, right.sourceSetMatch) ||
      right.commonPathDepth - left.commonPathDepth ||
      compareBoolean(left.fileNameExact, right.fileNameExact) ||
  ...
  static selectBestMatch(
    ...
    const scored = candidates.map((path) => ({
      path,
      rank: this.createMatchRank(referencePath, path),
    }));

    scored.sort((a, b) => this.compareMatchRanks(a.rank, b.rank) || a.path.localeCompare(b.path));
    return scored[0].path;
  }

  static selectUniqueBestMatch(referencePath: string, candidates: string[]): string | undefined {
    if (candidates.length === 0) return undefined;
    const scored = candidates.map(path => ({ path, rank: this.createMatchRank(referencePath, path) }));
    scored.sort((a, b) => this.compareMatchRanks(a.rank, b.rank) || a.path.localeCompare(b.path));
    return scored.length === 1 || this.compareMatchRanks(scored[0].rank, scored[1].rank) !== 0
      ? scored[0].path
      : undefined;
  }
  ...
  private static extractModuleRoot(filePath: string): string {
    const parts = this.normalizeSeparators(filePath).toLowerCase().split('/');
    const sourceIndex = parts.lastIndexOf('src');
    return sourceIndex > 0 ? parts.slice(0, sourceIndex).join('/') : '';
  }

  private static isSameModule(leftPath: string, rightPath: string): boolean {
    const left = this.extractModuleRoot(leftPath);
    const right = this.extractModuleRoot(rightPath);
    return left !== '' && left === right;
  }

  private static extractLogicalDirectoryParts(filePath: string): string[] {
    const parts = this.normalizeSeparators(filePath).toLowerCase().split('/');
    const sourceIndex = parts.findIndex(part => part === 'java' || part === 'resources');
    if (sourceIndex < 0) return parts.slice(0, -1);
    return parts.slice(sourceIndex + 1, -1).filter(part => part !== 'mapper' && part !== 'mappers');
  }

  private static extractSourceSet(filePath: string): string {
    const match = this.normalizeSeparators(filePath).toLowerCase()
      .match(/\/src\/(main|test)\/(?:java|resources)\//);
    return match?.[1] ?? '';
  }

  private static isSameSourceSet(leftPath: string, rightPath: string): boolean {
    const left = this.extractSourceSet(leftPath);
    const right = this.extractSourceSet(rightPath);
    return left !== '' && left === right;
  }
}
